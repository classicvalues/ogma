...
monitoring using Copilot:
```sh
$ ogma ros --variable-file variables --variable-db ros-variable-db --app-target-dir ros_demo
```

The application generated by Ogma contains the following files:
```
ros_demo/CMakeLists.txt
ros_demo/src/copilot_member_function.cpp
ros_demo/src/.keep
...
The handlers report monitor violations by printing messages to the logger. You
may be interested in defining a publisher and reporting violations that way.
